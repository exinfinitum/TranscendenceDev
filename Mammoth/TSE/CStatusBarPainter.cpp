...
//	Paint the bar. x, y are the coordinates for the top-center of the bar.
	...
	if (CG32bitImage::GetMasterRenderQueue())
		...
		int yPoint = yStart + (m_Options.cyBar / 2) + 1;
		Dest.DrawLine(xStart, yPoint, xStart + iFill, yPoint, m_Options.cyBar - 3, rgbFill);
		Dest.DrawLine(xStart + iFill, yPoint, xStart + m_Options.cxBar, yPoint, m_Options.cyBar - 3, rgbBlack);
		}
	else
		{
